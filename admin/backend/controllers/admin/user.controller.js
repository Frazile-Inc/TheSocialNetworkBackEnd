...
exports.getUsers = async (req, res) => {
  try {
    const { startDate, endDate, type, search, isBlock, gender, verified, country } = req.query;
    ...
    if (search && search !== "undefined") {
      ...
      searchQuery = {
        $or: [{ name: regex }, { userName: regex }, { uniqueId: regex }, { email: regex }],
      };
    }

    let typeQuery = {};
    if (type === "realUser") typeQuery = { isFake: false };
    else if (type === "fakeUser") typeQuery = { isFake: true };
    else if (type === "verifiedUser") typeQuery = { isVerified: true };
    else {
      return res.status(200).json({
        status: false,
    ...
    if (isBlock === "true") {
      filterQuery.isBlock = true;
    } else if (isBlock === "false") {
      filterQuery.isBlock = false;
    }
    ...
    if (country && country !== "All") {
      filterQuery.country = new RegExp(`^${country.trim()}$`, "i");
    }
    ...
    totalUsersPromise = User.countDocuments(finalQuery);

    usersPromise = User.find(finalQuery)
      .select("name userName coin image email country loginType ipAddress gender age uniqueId isBlock isFake isVerified createdAt lastlogin")
      .sort({ createdAt: -1 })
      .skip((start - 1) * limit)
      .limit(limit)
      .lean();

    const [totalUsers, users] = await Promise.all([totalUsersPromise, usersPromise]);
...
exports.manageUserCoin = async (req, res, next) => {
  try {
    const { userId, coin, action } = req.query;
    ...
    if (!["add", "deduct"].includes(action.trim().toLowerCase())) {
      return res.status(200).json({
        status: false,
    ...
    const [uniqueId, user] = await Promise.all([
      ...
      User.findById(userId).select("_id coin purchasedCoin").lean(),
    ]);
    ...
    await Promise.all([
      ...
      History.create({
        uniqueId: uniqueId,
        type: action.trim().toLowerCase() === "add" ? 6 : 7,
        userId,
        coin: coin,
    ...
    return res.status(200).json({
      status: true,
      message: `Successfully ${action.trim().toLowerCase() === "add" ? "added" : "deducted"} ${coin} coins.`,
    });
  } catch (error) {
