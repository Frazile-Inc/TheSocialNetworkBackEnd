...
exports.getUserProfile = async (req, res, next) => {
  try {
    const { userId: userIdStr, toUserId: toUserIdStr } = req.query;
    if (!userIdStr || !toUserIdStr) {
      return res.status(200).json({ status: false, message: "Oops! Invalid details!" });
    }

    const userId = new mongoose.Types.ObjectId(userIdStr);
    const toUserId = new mongoose.Types.ObjectId(toUserIdStr);

    const [user, followStatusAgg, totalLikesOfVideoPost, stories] = await Promise.all([
      User.findById(userId).lean(),
      FollowerFollowing.aggregate([
        {
          $facet: {
            isFollow: [
              { $match: { fromUserId: toUserId, toUserId: userId } },
              { $limit: 1 },
            ],
            totalFollowers: [
    ...
    const followData = followStatusAgg[0] || {};
    const isFollow = (followData.isFollow || []).length > 0;
    const totalFollowers = followData.totalFollowers?.[0]?.count || 0;
    const totalFollowing = followData.totalFollowing?.[0]?.count || 0;
...
//user login and sign up ( web )
exports.signInOrRegister = async (req, res) => {
  try {
    if (!req.body.identity || req.body.loginType === undefined || !req.body.fcmToken) {
    ...
    const loginType = Number(req?.body?.loginType);
    ...
    if (user) {
      ...
      if (req?.file) {
        const userImage = `${process.env.baseURL}${req?.file?.path}`;
...
//retrieves users sorted by followers ( web )
exports.getUsersByFollowers = async (req, res) => {
  try {
    const start = req.query.start ? parseInt(req.query.start) : 1;
    const limit = req.query.limit ? parseInt(req.query.limit) : 20;

    const usersWithFollowers = await FollowerFollowing.aggregate([
      {
        $group: {
          _id: "$toUserId",
    ...
    return res.status(200).json({
      status: true,
      message: "Users sorted by followers retrieved!",
...
//get user profile with total count of followers, total count of following and total count of videos and posts's likes (for guest user who is not login) ( web )
exports.getUserProfileByGuest = async (req, res, next) => {
  try {
    ...
    const [user, totalFollowers, totalFollowing, totalLikesOfVideoPost] = await Promise.all([
      User.findOne({ _id: userId }).lean(),
