      ...
      Video.aggregate([
        ...
        },
        {
          $addFields: {
            isLike: {
              $gt: [
                { $ifNull: [{ $arrayElemAt: ["$likeData.isLike", 0] }, 0] },
                0
              ]
            },
            totalLikes: {
              $ifNull: [{ $arrayElemAt: ["$likeData.totalLikes", 0] }, 0]
            },
            totalComments: {
              $ifNull: [{ $arrayElemAt: ["$commentsData.count", 0] }, 0]
            },
            totalViews: {
              $ifNull: [{ $arrayElemAt: ["$viewsData.count", 0] }, 0]
            },
          },
        },
        {
...
exports.getAllVideos = async (req, res, next) => {
  try {
    ...
    if (req.query.videoId) {
      ...
      const data = [
        ...
        },
        {
          $project: {
            ...
            totalLikes: { $ifNull: [{ $arrayElemAt: ["$totalLikes.count", 0] }, 0] },
      ...
      const [user] = await Promise.all([User.findOne({ _id: userId }).select("_id isBlock")]);
      ...
      const data = [
        ...
        // User's stories lookup (video owner)
        {
          $lookup: {
            from: "stories",
            localField: "userId",
...
exports.fetchVideosOfParticularSong = async (req, res) => {
  try {
    ...
    const [user, song, result] = await Promise.all([
      User.findOne({ _id: userId }).lean(),
            ...
            data: [
              ...
              },
              {
                $project: {
                  ...
                  totalLikes: { $ifNull: [{ $arrayElemAt: ["$totalLikes.count", 0] }, 0] },
                  totalComments: { $ifNull: [{ $arrayElemAt: ["$totalComments.count", 0] }, 0] },
                  totalViews: { $ifNull: [{ $arrayElemAt: ["$totalViews.count", 0] }, 0] },
                  songId: 1,
                  createdAt: 1,
    ...
    const totalVideosOfSong = result[0]?.total || 0;
...
exports.fetchUserVideos = async (req, res) => {
  try {
    if (!req.query.toUserId) {
      return res.status(200).json({ status: false, message: "toUserId must be required." });
    }

    const currentUserId = req.query.userId ? new mongoose.Types.ObjectId(req.query.userId) : null;

    const targetUserId = new mongoose.Types.ObjectId(req.query.toUserId);

    const targetUser = await User.findById(targetUserId).select("_id isBlock").lean();
    ...
    const matchCondition = {
      userId: targetUserId,
      ...(String(req.query.userId) !== String(req.query.toUserId)
        ? { isBanned: false }
        : {}),
    };
    ...
    const videos = await Video.aggregate([
      ...
      {
        $addFields: {
          isLike: currentUserId
            ? { $gt: [{ $size: "$likeHistory" }, 0] }
            : false,
...
exports.getVideoLibrary = async (req, res, next) => {
  try {
    ...
    const userId = req.query.userId && mongoose.Types.ObjectId.isValid(req.query.userId)
