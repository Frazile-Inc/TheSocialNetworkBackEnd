...
exports.sendGiftByUser = async (req, res) => {
  try {
    ...
    const [senderUser, video, gift] = await Promise.all([User.findById(userId).select("_id coin name").lean(), Video.findById(videoId).select("_id userId").lean(), Gift.findById(giftId).select("_id coin image").lean()]);
    ...
    if (receiverUser.fcmToken && !receiverUser.isBlock && receiverUser.fcmToken !== null) {
      ...
      adminPromise
        ...
        .then(async (response) => {
          ...
          notification.title = "🎁 Gift Received";
...
exports.sendGiftTolive = async (req, res) => {
  try {
    ...
    const [senderUser, gift, video] = await Promise.all([User.findById(userId).select("_id coin").lean(), Gift.findById(giftId).select("_id coin").lean(), LiveVideo.findById(videoId).select("_id userId").lean()]);
    const coin = Number(gift.coin);
