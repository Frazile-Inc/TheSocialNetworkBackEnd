...
const Owner = () => {
  ...
  const handleFileUpload = async (event: any) => {
    ...
    const response = await dispatch(uploadFile({ data: formData })).unwrap();

    if (response?.data?.status && response?.data?.url) {
      // ✅ local state — instant UI update
      setImagePath(response?.data?.url);
      ...
      <div className="profile-page p-0 payment-setting card1">
        <div className="cardHeader">
          <div className=" align-items-center d-flex flex-wrap justify-content-between p-3">
            <div>
              <p className="m-0 fs-5 fw-medium">Profile</p>
            </div>
          </div>
        </div>
        <div className="payment-setting-box  p-3">
          <div className="row">
            ...
            <div className={`${loginType === "admin" ? "col-lg-6 col-sm-12" : "col-12"}`}>
              <div className="mb-4">
