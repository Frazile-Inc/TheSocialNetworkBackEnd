...
export const allSong = createAsyncThunk(
  ...
  async (payload: AllSongPayload | undefined) => {
    ...
    if (payload?.startDate) query.append("startDate", payload.startDate);
    if (payload?.endDate) query.append("endDate", payload.endDate);
    if (payload?.start) query.append("start", payload.start.toString());
    if (payload?.limit) query.append("limit", payload.limit.toString());

    return apiInstanceFetch.get(`admin/song/getSongs?${query.toString()}`);
  }
);
...
export const allSongCategory = createAsyncThunk(
  ...
  async (payload: AllSongPayload | undefined) => {
    ...
    return apiInstanceFetch.get(`admin/songCategory/getSongCategory?${query.toString()}`);
  }
);
