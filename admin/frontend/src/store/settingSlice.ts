...
export const verifyPurchaseCode = createAsyncThunk(
  "admin/setting/validatePurchaseCode",
  async (payload: settingPayload | undefined) => {
    return apiInstanceFetch.get(`admin/setting/validatePurchaseCode`);
  }
);
...
const settingReducer = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(verifyPurchaseCode.pending, (state) => {
      state.isLoading = true;
    });

    builder.addCase(
      verifyPurchaseCode.fulfilled,
      (state, action: PayloadAction<any, string, { arg: settingPayload }>) => {
        state.isLoading = false;
        state.allowPaymentSettings = action.payload.status;
        state.purchaseCodeMessage = action.payload.message;
      }
    );

    builder.addCase(
      verifyPurchaseCode.rejected,
      (state, action: PayloadAction<any>) => {
        state.isLoading = false;
      }
    );
