...
exports.getAllPosts = async (req, res) => {
  try {
    ...
    if (req.query.postId) {
      ...
      const [user, post] = await Promise.all([User.findOne({ _id: userId }).select("_id isBlock").lean(), Post.findById(postId).select("_id").lean()]);
      ...
      const data = [
            ...
            pipeline: [
              {
                $group: {
                  _id: null,
                  count: { $sum: 1 },
                  isLike: { $max: { $cond: [{ $eq: ["$userId", userId] }, 1, 0] } },
                },
              },
            ],
            as: "likeData",
        ...
        },
        {
          $lookup: {
            ...
            as: "stories", // array of stories per post user
          },
        },
        {
          $project: {
            ...
            isLike: { $eq: [{ $arrayElemAt: ["$likeData.isLike", 0] }, 1] },
      ...
      const data = [
        ...
        },
        {
          $project: {
            ...
            isFake: 1,
            createdAt: 1,
            stories: 1, // include stories in the final output
            userId: "$user._id",
            name: "$user.name",
            userName: "$user.userName",
            userImage: "$user.image",
  ...
  try {
    ...
    const [user, posts] = await Promise.all([
      ...
      Post.aggregate([
        ...
        ...(req.query.userId.toString() === req.query.toUserId.toString()
