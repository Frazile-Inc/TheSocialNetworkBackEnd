...
exports.uploadStory = async (req, res) => {
  try {
    ...
    const [user, backgroundSong] = await Promise.all([
      User.findOne({ _id: req.body.userId, isFake: false }).select("_id isBlock").lean(),
      req.body.backgroundSong ? Song.findById(req.body.backgroundSong).select("_id").lean() : null, // Only fetch backgroundSong if provided
    ]);
...
exports.viewStory = async (req, res) => {
  try {
    ...
    const [user, story, alreadyViewed] = await Promise.all([User.findOne({ _id: userId, isFake: false }).select("_id isBlock").lean(), Story.findById(storyId).select("_id").lean(), StoryView.findOne({ userId, storyId }).select("_id").lean()]);
...
exports.reactToStory = async (req, res) => {
  try {
    ...
    const [user, story, existingReaction] = await Promise.all([
      ...
      Story.findById(storyId).populate("user").select("_id user").lean(), // get story + owner
      StoryReaction.findOne({ userId, storyId }).select("_id").lean(),
    ]);
...
exports.deleteStory = async (req, res) => {
  try {
    ...
    await Promise.all([StoryView.deleteMany({ storyId: storyId }), StoryReaction.deleteMany({ storyId: storyId }), Story.deleteOne({ _id: storyId })]);
  } catch (error) {
...
exports.getFollowedUserStories = async (req, res) => {
  try {
    ...
    const [user] = await Promise.all([User.findOne({ _id: userId, isFake: false }, "_id isBlock").lean()]);
    ...
    const realStoriesPipeline = [
      ...
      },
      {
        $lookup: {
          ...
          pipeline: [
            { $group: { _id: null, count: { $sum: 1 }, isViewed: { $max: { $cond: [{ $eq: ["$userId", new mongoose.Types.ObjectId(userId)] }, 1, 0] } } } }
          ],
          as: "viewsData",
        },
      },
      {
        $addFields: {
          viewsCount: { $ifNull: [{ $arrayElemAt: ["$viewsData.count", 0] }, 0] },
          isView: { $gt: [{ $ifNull: [{ $arrayElemAt: ["$viewsData.isViewed", 0] }, 0] }, 0] },
        },
      },
      {
        $lookup: {
      ...
      {
        $addFields: {
          reactionsCount: { $ifNull: [{ $arrayElemAt: ["$reactionsData.count", 0] }, 0] },
        },
      },
      {
        $project: {
    ...
    combinedStories.sort((a, b) => {
      ...
      // real first
      if (a.user.isFake !== b.user.isFake) {
        return a.user.isFake - b.user.isFake;
      }
    ...
    const paginatedStories = combinedStories.slice(skip, skip + limit);

    res.status(200).json({
      status: true,
      message: "Stories from followed users fetched successfully.",
      storyGroup: paginatedStories,
    });
