...
exports.getSongsByUser = async (req, res, next) => {
  try {
    ...
    const [user, songResult] = await Promise.all([
            ...
            data: [
              ...
              },
              {
                $project: {
                  ...
                  isFavorite: { $gt: [{ $ifNull: [{ $arrayElemAt: ["$favoriteData.isFavorite", 0] }, 0] }, 0] },
                },
              },
            ],
          },
    ...
    const total = songResult[0]?.metadata[0]?.total || 0;
    const songs = songResult[0]?.data || [];

    return res.status(200).json({ status: true, message: "Retrieve the list of songs.", total: total, songs: songs });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ status: false, message: error.message || "Internal Server Error" });
  }
