  ...
  socket.on("disconnect", async (reason) => {
    ...
    if (globalRoom) {
      ...
      if (socket?.length == 0) {
        ...
        if (user) {
          // Check if the user was a viewer in a live room
          try {
            const viewerEntry = await LiveView.findOne({ userId }).lean();

            if (!viewerEntry) {
              console.log(`[Disconnected user] User ${userId} was not a viewer in any live`);
            }

            const { liveHistoryId } = viewerEntry;

            console.log(
              `[Disconnected user] Removing user ${userId} from LiveView of liveHistoryId ${liveHistoryId}`
            );

            await LiveView.deleteOne({ _id: viewerEntry._id });

            const [updatedViewCount] = await Promise.all([
              LiveView.countDocuments({ liveHistoryId }),
            ]);

            await LiveUser.updateOne(
              { liveHistoryId },
              { $set: { view: updatedViewCount } }
            );

            // socket.leave(liveHistoryId.toString());
            io.in(liveHistoryId.toString()).emit("lessView", updatedViewCount);

            const socketCount =
              io.sockets.adapter.rooms.get(liveHistoryId.toString())?.size || 0;

            console.log(
              `[Disconnected user] Emitted lessView (${updatedViewCount}) to ${socketCount} sockets`
            );
          } catch (err) {
            console.error("❌ Error while handling lessView on disconnect:", err);
          }

          const liveUserDoc = await LiveUser.findOne({ userId: user._id });
          if (liveUserDoc?.isPkMode && liveUserDoc?.pkConfig) {
            console.log("PK running - ending due to disconnect");

            const pk = liveUserDoc.pkConfig;
            const isHost1 = pk.host1Id?.toString() === user._id.toString();
            const opponentId = isHost1 ? pk.host2Id : pk.host1Id;

            const opponentLiveUser = await LiveUser.findOne({ userId: opponentId }).select("liveHistoryId");
            console.log("opponentLiveUser in disconnect globalRoom: ", opponentLiveUser);

            if (user.liveHistoryId) {
              io.in(user.liveHistoryId.toString()).emit("pkEndedByDisconnect", {
                reason: "disconnect",
              });

              const sockets = await io.in(user.liveHistoryId.toString()).fetchSockets();
              console.log("sockets found in pkEndedByDisconnect [1]", sockets?.length);
            }

            if (opponentLiveUser?.liveHistoryId) {
              io.in(opponentLiveUser.liveHistoryId.toString()).emit("pkEndedByDisconnect", {
                reason: "opponent_disconnect",
              });

              const sockets = await io.in(opponentLiveUser.liveHistoryId.toString()).fetchSockets();
              console.log("sockets found in pkEndedByDisconnect [2]", sockets?.length);
            }
          }
